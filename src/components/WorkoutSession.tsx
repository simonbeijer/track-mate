...
export const WorkoutSession: React.FC<WorkoutSessionProps> = ({
  ...
  const toggleOption = (
    ...
    option: "showSets" | "showWeight" | "showReps"
  ) => {
    setExerciseStates((prev) => ({
      ...prev,
      [exerciseId]: {
  ...
  const updateSet = (
    exerciseId: string | number,
    setIndex: number,
    data: Partial<{ done: boolean; weight?: number; reps?: number }>
  ) => {
    setExerciseStates((prev) => {
      ...
      const allDone = updatedSets.every((set) => set.done);
          ...
          return (
            <Card
              ...
              className={`shadow-card transition-all ${
                exerciseState.done ? "border-accent bg-accent/5" : ""
              }`}
            >
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle
                        className={`text-lg ${
                          exerciseState.done ? "line-through" : ""
                        }`}
                        style={
                          exerciseState.done
                            ? { color: "hsl(215 16% 47%)" }
